...
test "pow" {
    {
        ...
        try testing.expect(base.pow(3).eql(&FieldElement{ .prime = 13, .num = 1 }));
    }
    {
        const base = FieldElement{ .prime = 31, .num = 17 };
        try testing.expect(base.pow(-3).eql(&FieldElement{ .prime = 31, .num = 29 }));
    }
}
